...
import (
	...
	"github.com/openyurtio/pool-coordinator/utils/fs"
)
...
const (
	CertDir = "/tmp/pool-coordinator/serving-certs/"
)
...
func (c *Certs) generate(serviceNamespace, serviceName string) error {
	...
	caCert, err := cert.NewSelfSignedCACert(cert.Config{CommonName: "webhooks-cert-ca"}, caKey)
	if err != nil {
	...
	err = fs.EnsureDir(CertDir)
	if err != nil {
		return err
	}
	err = fs.WriteFile(CertDir+ServerCertName2, c.Cert)
	if err != nil {
		return err
	}
	err = fs.WriteFile(CertDir+ServerKeyName2, c.Key)
	if err != nil {
		return err
	}
